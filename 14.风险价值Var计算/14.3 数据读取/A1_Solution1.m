...
CSI300Dates=txt(4:end,1);%时间 %注：第4行到最后一行，第一列（时间）
...
save CSI300Prices CSI300Dates CSI300Tickers CSI300HistPrices positionsPortfolio pricesIndex %注：文件名 数据1 数据2 。。。
...
% 可视化价格序列
% 标准化价格，初始价格为1.00
normPrices = ret2tick(tick2ret(CSI300HistPrices));%注：第一天的数据作为1；tick2ret：计算价格序列对应的收益率序列；ret2tick：收益率→价格（初始值为1）。价格→收益率→价格：起到了Norm 1标准化的作用。

% 绘制选定股票的标准化价格，'万科A'，'潍柴动力'，'上海能源'
%选定股票 %注：很巧妙，用1所在的位置取值
...
corrcoef(mypickRet)% 相关性 %注：相关系数矩阵（协方差矩阵计算函数：cov）

% 简单Beta计算
IndexRet = tick2ret(pricesIndex);%指数收益率
SZ02 = tick2ret(mypickStockPrices(:,1));%选中股票价格转为收益率 %注：万科
...
returnsSecurity = tick2ret(CSI300HistPrices,[],'Continuous');%注：所有股票的
%累计收益率
totalReturns = sum(returnsSecurity);%注：矩阵默认按列计算
numDays = size(CSI300HistPrices, 1);%注：第1列的行数
