...
% 历史模拟方法，计算投资组合价值
pricesPortfolio = CSI300HistPrices*positionsPortfolio;%注：每天的投资组合价值=当天的所有股票的sum（股票价格*股票数量）

% 投资组合的收益率
returnsPortfolio = tick2ret(pricesPortfolio, [], 'continuous');%注：If Method is 'Continuous', the returns are: RetSeries(i) = log[TickSeries(i+1)/TickSeries(i)]

% 投资组合最后一日的市值　%注：最后一天的市值才是最后的市值
