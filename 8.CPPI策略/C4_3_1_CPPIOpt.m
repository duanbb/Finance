...
for i=1:testNum
    ...
    SDataMatrix(i,:)=[Price0;SData];%注：给第i行赋值
end
...
for testNo=1:length(CPPITestMatrix)
    for i=1:testNum
        ...
        testReturn(i)=( A(TradeDayTimeLong+1)-A(1) )/A(1);%注：（最后一次-初始）/初始
        testVolatility(i)=std( price2ret( A ) )*sqrt(TradeDayOfYear);%注： 将价格序列改为收益率序列 （价格转化为增长率）。波动指的是收益率的波动。
    end
